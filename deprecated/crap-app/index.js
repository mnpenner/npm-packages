...
async function main(args) {
    ...
    if (license !== 'UNLICENSED') {
        const licenseText = await readFile(Path.join(__dirname, 'licenses', license), {
            encoding: 'utf8',
        })
        await writeFile(
            Path.join(outputDir, 'LICENSE'),
            replaceMulti(licenseText, {
    ...
    const readmeContents = await readFile(Path.join(__dirname, 'template', 'README.md'), {
        encoding: 'utf8',
    })
    await writeFile(
        Path.join(outputDir, 'README.md'),
        readmeContents.replace(/\{\{\s*project\s*\}\}/giu, pkgName),
    )
