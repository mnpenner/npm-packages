...
function logResponse(reqId: number, elapsed: number, res: Response) {
    console.log(
        Chalk.grey(`<${reqId}`) +
            ` ${formatStatus(res.status)} | ${formatSize(res.headers.get('content-length'))} | ${formatContentType(res.headers.get('content-type'))} | ${formatDuration(elapsed)}`,
    )
}
...
export function formatDuration(duration: number) {
    ...
    return Chalk.red(
        Intl.NumberFormat(undefined, { maximumFractionDigits: 2 }).format(duration / 1000) + 's',
    )
}
