...
async function main(options: Options, _positionals: Positionals): Promise<number | void> {
            ...
            if (status === 'fail') {
                ...
                if (failingTests.length > 0) {
                    ...
                    if (uniqueFailing.length > 5) {
                        console.log(
                            chalk.gray(`  ... and ${uniqueFailing.length - 5} more failing tests`),
                        )
                    }
                } else if (exitCode !== 0) {
                    const errorLines = lines.filter(
                        (l) => l.toLowerCase().includes('error:') || l.includes('EPIPE'),
                    )
    ...
    const workers = Array.from({ length: Math.min(CONCURRENCY, targetPackages.length) }, () =>
        runNext(),
    )
