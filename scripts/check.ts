...
async function main(_options: Options, positionals: Positionals): Promise<number | void> {
    ...
    if (positionals.length === 0) {
        ...
        const [testRes, lintRes, typeRes] = await Promise.all([testPromise, lintPromise, typecheckPromise]);
    ...
    const targetedDisplay = Array.from(targetPackages).join(", ") || (positionals.length > 0 ? "targeted files" : "all");
