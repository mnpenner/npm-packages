add a `tsdown.config.ts` to packages/routekit and packages/rerouter. they should look something like this:
...
likewise, add "ex:gen" for routekit. and then an "ex:gen" in the root package.json that will regenerate all the examples.
...
- "test:gen" which runs "gen" and then _tests_ the generated code (we'll eventually have a .test.ts which imports the generated code)
- "test" runs all tests.
