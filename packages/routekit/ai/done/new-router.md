in packages/routekit/src/router.ts, simplify fetch(). in packages/routekit/src/types.ts, the Handler should take as an argument
...
params for packages/routekit/src/bin/gen-api-client.ts. even if fetch() doesn't use them, the user needs to be able to specify them
for type inference.
