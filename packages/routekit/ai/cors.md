Implement a CORs middleware at packages/routekit/src/middleware/cors.ts inspired by other frameworks but adapted for this project. Maybe add a dev mode like packages/routekit/src/middleware/csrf.ts if it'll be helpful. If you need to share any functions with that file, move the helpers under packages/routekit/src/lib/.
