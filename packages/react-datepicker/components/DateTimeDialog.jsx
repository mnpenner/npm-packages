...
import { cc as classnames } from '@mpen/classcat'
import SunCalc from 'suncalc'

moment.locale('en-ca')
const localeData = moment.localeData()
const months = localeData.monthsShort()
const monthChunks = lo.chunk(
    months.map((m, n) => [n, m]),
    3,
)
...
const skyColors = ['#1F252D', '#263E66', '#4773BB', '#87A4D3', '#DBE9FF']

const tzDataLoaders = import.meta.glob('../data/*.txt', { query: '?raw', import: 'default' })
...
export default class DateTimeDialog extends React.Component {
    ...
    constructor(props) {
        ...
        if (this.state.loading) {
            const loadTzData = tzDataLoaders[`../data/${props.timezone}.txt`]
            if (loadTzData) {
                loadTzData().then((tzData) => {
    ...
    render() {
        ...
        for (let d = 1; d <= lastDayNumber; ++d) {
            row.push(
                <td
                    key={d}
                    className={classnames(cn.dayBtn, { [cn.daySelected]: this.state.day === d })}
                    onClick={this.clickDay(d)}
                >
                    {d}
                </td>,
            )
            ...
            <div>
                <div className={cn.root}>
                    <div className={cn.header}>
                        <div>{moment.tz(selectedDate, this.props.timezone).format('LLLL (z)')}</div>
                    </div>
                    <div className={cn.body}>
                        <div className={cn.column}>
                            <div className={cn.yearContainer}>
                                <a className={cn.yearBtn} onClick={this.incYear(-10)}>
                                    &laquo;
                                </a>
                                <a className={cn.yearBtn} onClick={this.incYear(-1)}>
                                    &lt;
                                </a>
                                <input
                                    type="number"
                                    className={cn.yearInput}
                                        ...
                                        <tr key={idx}>
                                            {chunk.map(([m, mmm]) => (
                                                <td
                                                    ...
                                                    className={classnames(cn.monthBtn, {
                                                        [cn.monthSelected]: this.state.month == m,
                                                    })}
                                                >
                                                    {mmm}
                                                </td>
                        ...
                        <div className={cn.column}>
                            <div
                                className={cn.pmCol}
                                style={{
                                    background: `linear-gradient(to bottom,${pmGradient.join(',')})`,
                                }}
                            >
                                <div
                                    className={cn.amCol}
                                    style={{
                                        background: `linear-gradient(to bottom,${amGradient.join(',')})`,
                                    }}
                                >
                                        ...
                                        <tbody>
                                            {lo.times(12, (i) => {
                                                ...
                                                return (
                                                    <tr key={i}>
                                                        <td
                                                            className={classnames(cn.hourCell, {
                                                                [cn.timeSelected]:
                                                                    this.state.hour === i,
                                                            })}
                                                        >
                                                            <a
                                                                onClick={this.clickHour(i)}
                                                                className={cn.timeBtn}
                                                                href=""
                                                            >
                                                                {disp}
                                                                <sup>am</sup>
                                                        ...
                                                        <td
                                                            className={classnames(cn.hourCell, {
                                                                [cn.timeSelected]:
                                                                    this.state.hour === i + 12,
                                                            })}
                                                        >
                                                            <a
                                                                onClick={this.clickHour(i + 12)}
                                                                className={cn.timeBtn}
                                                                href=""
                                                            >
                                                                {disp}
                        ...
                        <div className={cn.column}>
                            <div
                                className={cn.minGradient}
                                style={{
                                    background: `linear-gradient(to bottom,${minGradient.join(',')})`,
                                }}
                            >
                                    ...
                                    <tbody>
                                        {minuteValues.map((i) => {
                                            ...
                                            return (
                                                <tr key={i}>
                                                    <td
                                                        className={classnames(cn.minCell, {
                                                            [cn.timeSelected]:
                                                                this.state.minute === i,
                                                            [cn.halfHour]: i % 30 === 0 || i === 59,
                                                            [cn.quarterHour]:
                                                                i % 15 === 0 || i === 59,
                                                        })}
                                                    >
                                                        <a
                                                            onClick={this.clickMinute(i)}
