 ...
 * Set a key in a Map.
 */
export function mapSet<K, V>(map: Map<K, V>, key: K, value: Resolvable<V, [V | undefined]>): Map<K, V> {
    const ret = new Map(map)
    ret.set(key, resolveValue(value, map.get(key)))
    return ret
}
...
export function fpMergeMap<K, V>(values: Resolvable<Iterable<readonly [K, Resolvable<V, [V | undefined]>]>,[Map<K, V>]>) {
    return (map: Map<K, V>) => {
        const ret = new Map(map)
