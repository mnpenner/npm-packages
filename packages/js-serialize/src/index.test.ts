...
expect.extend({
    toWse(received: unknown, expected: string) {
        ...
        return {
            pass,
            message: () =>
                `expected string ${pass ? 'not' : ''} to be whitespace-normalized equal to "${expected}" but got "${normalized}"`,
        }
    },
})
...
describe('strings', () => {
    ...
    it('uses single-character escape sequences', () => {
        expect(jsSerialize('\b\f\n\r\t\v"\\', { safe: false })).toEqual(
            "'\\b\\f\\n\\r\\t\\v\"\\\\'",
        )
        expect(jsSerialize('\b\f\n\r\t\v"\\', { safe: true })).toEqual(
            "'\\b\\f\\n\\r\\t\\x0B\"\\\\'",
        )
    })
...
it('serializes dates', () => {
    ...
    expect(jsSerialize(new Date('2017-05-04T16:55:50.457Z'), { compact: false })).toEqual(
        'new Date(Date.UTC(2017,4,4,16,55,50,457))',
    )
    expect(jsSerialize(new Date('2017-05-04T16:55:50.457Z'), { compact: true })).toEqual(
        'new Date(1493916950457)',
    )
    expect(jsSerialize(new Date(1642572794510), { compact: true })).toEqual(
        'new Date(1642572794510)',
    )
    expect(jsSerialize(new Date(1642572794510), { compact: false })).toEqual(
        'new Date(Date.UTC(2022,0,19,6,13,14,510))',
    )
    expect(jsSerialize(new Date(1642572794000), { compact: false })).toEqual(
        'new Date(Date.UTC(2022,0,19,6,13,14))',
    )
    expect(jsSerialize(new Date(Date.UTC(2022, 0, 19)), { compact: false })).toEqual(
        'new Date(Date.UTC(2022,0,19))',
    )
    expect(jsSerialize(new Date(Date.UTC(2022, 0, 19)), { compact: true })).toEqual(
        'new Date(1642550400000)',
    )
})

it('serializes scripts', () => {
    expect(jsSerialize('<script>alert("injection")</script>')).toEqual(
        '\'\\x3cscript\\x3ealert("injection")\\x3c/script\\x3e\'',
    )
    expect(jsSerialize(() => document.write('</ScRiPt >'))).toEqual(
...
it('serializes arrays', () => {
    ...
    expect(jsSerialize([undefined, undefined, undefined, undefined])).toEqual(
...
describe('functions', () => {
    it('serializes native functions', () => {
        ...
        expect(jsSerialize(Intl.NumberFormat.supportedLocalesOf)).toEqual(
    ...
    it('throws on unknown native functions', () => {
        ...
        expect(() => jsSerialize(fn)).toThrow(
            "Could not determine fully-qualified name of native function 'unknown'",
        )
    })
})
...
describe('objects', () => {
    it('serializes objects', () => {
        ...
        expect(jsSerialize({ 'foo bar': 'baz', corge: 'Waldo' })).toEqual(
            '{"foo bar":"baz",corge:"Waldo"}',
        )
    ...
    it('quotes keyword property names', () => {
        ...
        ).toEqual(`{"class":1,"do":2,"finally":3,"for":4,five:5}`)
        expect(
            jsSerialize(
                {
                    class: 1,
...
describe('recursion', () => {
    ...
    it('serializes recursive sets', () => {
        ...
        // console.log(input)
        expect(jsSerialize(input, { safe: false })).toEqual(
            '(($0,$1)=>({a:($0={},Object.assign($0,{a:"perfect",circle:$0})),b:($1=[],$1.push($0,1,$0),$1),c:{d:$1,e:$0,f:new Set([$0])}}))()',
        )
    })

    it('serializes recursive objects', () => {
        const foo = { foo: 1 }
        expect(jsSerialize({ a: foo, b: foo }, { safe: false })).toEqual(
            '($0=>({a:($0={},Object.assign($0,{foo:1})),b:$0}))()',
        ) // shorter: (($0)=>({a:$0,b:$0}))({foo:1})

        const o: any = { a: 'perfect' }
        o.circle = o
        expect(jsSerialize({ apc: o }, { safe: false })).toEqual(
            '($0=>({apc:($0={},Object.assign($0,{a:"perfect",circle:$0}))}))()',
        )
...
describe('references', () => {
    ...
    it('deduplicates long strings', () => {
        expect(jsSerialize(['12345678901', 'abc', '12345678901'])).toEqual(
            '["12345678901","abc","12345678901"]',
        )
        expect(jsSerialize(['123456789012', 'abc', '123456789012'])).toEqual(
            '($0=>[$0="123456789012","abc",$0])()',
        )
    })
    ...
    it('serializes object recursive sealed references', () => {
        ...
        o.ref = o
        Object.seal(o)
        expect(jsSerialize([o, o])).toEqual(
            '($0=>[($0={},Object.seal(Object.assign($0,{ref:$0}))),$0])()',
        )
    })
