...
describe('allSettled', () => {
    it('returns an array of SyncResults and never Errs', async () => {
        const settled = await allSettled([nj(1), nj(err('boom')), Promise.resolve('ok')] as const)
...
describe('allOk', () => {
    it('returns Ok with array of values when all succeed', async () => {
        ...
        expectType<
            TypeEqual<typeof combined, Result<readonly [number, string], DetailedError<unknown>>>
        >(true)
