...
describe('firstSettled', () => {
    it('yields the earliest result regardless of success or failure', async () => {
        ...
        expectType<
            TypeEqual<typeof promise, NeverjectPromise<number, string | DetailedError<unknown>>>
        >(true)
...
describe('firstOk', () => {
    it('resolves on the first Ok even if Errs arrive earlier', async () => {
        ...
        const slowerErr = nj(
            new Promise<Result<number, string>>((resolve) =>
                setTimeout(() => resolve(err('late fail')), 15),
            ),
        )
        ...
        const settled = await promise
        expectType<TypeEqual<typeof settled, Result<number, (string | DetailedError<unknown>)[]>>>(
            true,
        )
    ...
    it('collects all errors when no input succeeds', async () => {
        ...
        expectType<
            TypeEqual<typeof allFail, NeverjectPromise<string, (string | DetailedError<unknown>)[]>>
        >(true)

        const settled = await allFail
        expectType<TypeEqual<typeof settled, Result<string, (string | DetailedError<unknown>)[]>>>(
            true,
        )
        expect(settled.ok).toBe(false)
        if (settled.ok) return
