...
function normalizeResult<V, E>(value: NormalizedValue<V, E>): Promise<Result<V, E | DetailedError<unknown>>> {
    return Promise.resolve(value)
        .then((resolved) => resolve(resolved) as Result<V, E | DetailedError<unknown>>)
        .catch((reason) => rejectWithError(reason) as Result<V, DetailedError<unknown>>)
}

function normalizeErrResult<V, E>(value: NormalizedError<V, E>): Promise<Result<V, E | DetailedError<unknown>>> {
    return Promise.resolve(value)
        .then((resolved) => reject(resolved) as Result<V, E | DetailedError<unknown>>)
        .catch((reason) => rejectWithError(reason) as Result<V, DetailedError<unknown>>)
}
...
export class NeverjectPromise<V, E> implements PromiseLike<Result<V, E>> {
    ...
    /**
     * Await this [`NeverjectPromise`]{@link NeverjectPromise} as you would a native promise, receiving a [`Result`]{@link Result}. Prefer chaining with [`map`]{@link NeverjectPromise#map} or [`mapErr`]{@link NeverjectPromise#mapErr} to keep error handling consistent.
     *
     ...
     * @param onrejected - Runs if the underlying promise unexpectedly rejects, receiving an [`UnreachableError`]{@link UnreachableError}.
    ...
    /**
     * Transform a successful value, flattening returned [`Result`]{@link Result} or [`NeverjectPromise`]{@link NeverjectPromise} instances while preserving errors.
     *
     * @typeParam NV - Mapped success type.
     * @typeParam NE - Error type produced by the mapper.
     * @param onfulfilled - Receives the successful value and may return a new value, [`Result`]{@link Result}, promise, or [`NeverjectPromise`]{@link NeverjectPromise}.
     * @param onrejected - Optional mapper for the error branch; may produce a value, [`Result`]{@link Result}, promise, or [`NeverjectPromise`]{@link NeverjectPromise}.
     ...
     */
    map<NV, NE = E>(onfulfilled: (value: V) => NormalizedValue<NV, NE>, onrejected?: (error: E) => NormalizedValue<NV, NE>): NeverjectPromise<NV, E | NE | DetailedError<unknown>> {
        const promise = this.promise.then(async (result) => {
            if(result.ok) {
                try {
                    return await normalizeResult<NV, NE>(onfulfilled(result.value))
                } catch (error) {
                    return rejectWithError(error) as Result<NV, DetailedError<unknown>>
                }
            }
     ...
     * @param fn - Receives the error payload and may return an error, [`Result`]{@link Result}, promise, or [`NeverjectPromise`]{@link NeverjectPromise}.
     ...
     */
    mapErr<NE>(fn: (error: E) => NormalizedError<V, NE>): NeverjectPromise<V, NE | DetailedError<unknown>> {
        const promise = this.promise.then(async (result) => {
            if(result.ok) return result as Result<V, NE | DetailedError<unknown>>
     ...
     * @typeParam NE - New error type produced by `fn`.
     * @param fn - Receives the settled [`Result`]{@link Result} and may return another [`Result`]{@link Result}, promise, or [`NeverjectPromise`]{@link NeverjectPromise}.
     ...
     */
    mapResult<NV, NE = E>(fn: (result: Result<V, E>) => NormalizedValue<NV, NE>): NeverjectPromise<NV, NE | DetailedError<unknown>> {
        const promise = this.promise.then(async (result) => {
            try {
    ...
    /**
     * Resolve to the successful value or a fallback when this settles as [`Err`]{@link Err}, mirroring [`Err.valueOr`]{@link Err#valueOr}.
     *
     ...
     * @param fn - Callback receiving the settled [`Result`]{@link Result}; may return void, [`Result`]{@link Result}, promise, or [`NeverjectPromise`]{@link NeverjectPromise}.
    ...
    /**
     * Handle an error branch by producing a recovery value or mapped [`Result`]{@link Result}, leaving successful values untouched.
     *
     * @typeParam RV - Recovery success type.
     * @typeParam NE - Error type produced by the recovery handler.
     * @param fn - Callback invoked when this settles as [`Err`]{@link Err}; may return a value, [`Result`]{@link Result}, promise, or [`NeverjectPromise`]{@link NeverjectPromise}.
     * @returns A [`NeverjectPromise`]{@link NeverjectPromise} that preserves successes and replaces failures with the handler output.
     * @example
     * const recovered = await nj(err('offline')).recover(() => 'cached')
     * console.assert(recovered.ok && recovered.value === 'cached')
     */
    recover<RV, NE = E>(fn: (error: E) => NormalizedValue<RV, NE>): NeverjectPromise<V | RV, E | NE | DetailedError<unknown>> {
        const promise = this.promise.then(async (result) => {
            ...
            try {
                ...
                return rejectWithError(error) as Result<RV, DetailedError<unknown>>
            }
        })
