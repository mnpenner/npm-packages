...
describe('Utilities', () => {
    ...
    example('Grabbing earliest success with firstOk', async () => {
        ...
        const maybeWin = (label: string, delay: number) =>
            new Promise((resolve) =>
                setTimeout(() => resolve(flip() ? ok(label) : err(`${label} failed`)), delay),
            )

        const result = await njAgg.firstOk([
            maybeWin('fast attempt', 5),
            maybeWin('slow attempt', 20),
        ] as const)
        ...
        const retries = await njAgg.firstOk([
            maybeWin('retry 1', 10),
            maybeWin('retry 2', 15),
        ] as const)
