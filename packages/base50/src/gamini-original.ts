...
try {
    ...
    // Verification
    const isEqual =
        originalData.length === decodedData.length &&
    ...
    console.log(
        'Single Byte:',
        uint8ArrayToBase50(singleByte),
        base50ToUint8Array(uint8ArrayToBase50(singleByte)),
    )
    ...
    console.log(
        'Large Decoded Matches:',
        largeNum.every((v, i) => v === largeDecoded[i]),
    )
