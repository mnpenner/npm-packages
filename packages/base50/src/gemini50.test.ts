...
describe('Base50 Streaming Conversion', () => {
    ...
    describe('base50ToUint8ArrayStreaming (Decode)', () => {
        ...
        it('should throw an error for invalid characters', () => {
            ...
            expect(() => base50ToUint8ArrayStreaming(input)).toThrow(
                `Invalid Base50 character found: "${invalidChar}"`,
            )
        })
        ...
        it('should throw an error for characters outside the alphabet (e.g., space, symbols)', () => {
            const input1 = 'bc df'
            const input2 = 'bc+df'
            expect(() => base50ToUint8ArrayStreaming(input1)).toThrow(
                'Invalid Base50 character found: " "',
            )
            expect(() => base50ToUint8ArrayStreaming(input2)).toThrow(
                'Invalid Base50 character found: "+"',
            )
        })
    })
    ...
    describe.skipIf(!Bun.env.RUN_SLOW_TESTS)('Round Trip Tests', () => {
        ...
        it(
            'should correctly encode and decode 10,000 random arrays of varying sizes using crypto.getRandomValues',
            () => {
                ...
                if (typeof crypto === 'undefined' || typeof crypto.getRandomValues !== 'function') {
                    console.warn(
                        'Web Crypto API (crypto.getRandomValues) not available in this environment. Skipping randomized test.',
                    )
                ...
                for (let i = 0; i < numTests; i++) {
                    ...
                    if (randomSize > 0) {
                        // getRandomValues doesn't work on 0-length arrays
                    ...
                    try {
                        ...
                        console.error(
                            `Random test failed at iteration ${i} with size ${randomSize}`,
                        )
                        console.error('Original Data (first 20 bytes):', originalData.slice(0, 20))
                        if (typeof encoded! !== 'undefined') {
                            console.error(
                                'Encoded String (first 30 chars):',
