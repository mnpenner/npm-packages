...
process.on('uncaughtException', async (err) => {
    ...
    const match = errStr.match(
        /^ {4}at (?:(?<func>.+?) \((?<file>.+?):(?<line>\d+):(?<col>\d+)\)|(?<file2>.+?):(?<line2>\d+):(?<col2>\d+))$/m,
    )
    ...
    console.log(
        Chalk.bgRed(` ${errorType?.[1] ?? 'Error'} `) +
            Chalk.white(
                ` ${Path.relative(process.cwd(), topfilename)}:${Chalk.blue(match.groups!.line ?? match.groups!.line2)}:${Chalk.cyan(match.groups!.col ?? match.groups!.col2)}`,
            ),
    )
    ...
    const stackTrace = Array.from(
        stackStr.matchAll(
            /^ {4}at (?:(?<func>.+?) \((?<file>.+?):(?<line>\d+):(?<col>\d+)\)|(?<file2>.+?):(?<line2>\d+):(?<col2>\d+))$/gm,
        ),
    )
    ...
    for (const frame of stackTrace) {
        ...
        // console.log(line);
        console.log(
            `${Chalk.bold(`${String(--lineCount).padStart(width, ' ')}.`)} ${Chalk.underline(filename)}:${Chalk.blue(frame.groups!.line ?? frame.groups!.line2)}:${Chalk.cyan(frame.groups!.col ?? frame.groups!.col2)}${frame.groups!.func ? `  ${Chalk.yellow(frame.groups!.func)}` : ''}`,
        )
    }
