...
describe(runChain.name, () => {
    ...
    const doubleItAndImplicitlyPassItOn: TransformInputImplicitReturnFn<number> = async (
        ctx,
        next,
    ) => {
        await next(ctx * 2)
    ...
    const stringifyOutput: TransformOutputFn<number, string> = async (ctx, next) =>
        String(await next(ctx))
    const modifyContext =
        (num: number): AddContext<{ data: number }> =>
        (ctx) => {
            ctx.data = num
        }
    ...
    test('2 fn chain', async () => {
        expect(await runChain(1, [doubleItAndPassItOn, addThreeAndTakeIt])).toBe(5)
    })

    test('implicit pass', async () => {
        expect(await runChain(1, [doubleItAndImplicitlyPassItOn, addThreeAndTakeIt])).toBe(5)
    })
    ...
    test('short circuit', async () => {
        expect(await runChain(1, [addThreeAndTakeIt, doubleItAndPassItOn])).toBe(4)
    })

    test('first fn determines output type', async () => {
        expect(
            await runChain<number, string>(1, [
                stringifyOutput,
                doubleItAndPassItOn,
                addThreeAndTakeIt,
            ]),
    ...
    test('transform input', async () => {
        expect(await runChain(1, [doubleItAndPassItOn, stringifyInput, doubleCat])).toBe('22')
    })

    test('modify context', async () => {
        expect(await runChain({}, [modifyContext(123), readContext])).toBe(123)
    })
})
