...
const AnyOf = <T extends any[]>(values: [...T], options?: SchemaOptions) =>
    Type.Union(
        values.map((val) => Type.Literal(val)),
        options,
    )

type SetTypeOptions = Omit<ArrayOptions, 'uniqueItems'>

const StringSet = (options: SetTypeOptions) =>
...
const DbIndexType = AnyOf(['BTREE', 'INDEX', 'PRIMARY', 'UNIQUE', 'HASH'])

const DbReferenceOption = AnyOf(['CASCADE', 'NO ACTION', 'RESTRICT', 'SET DEFAULT', 'SET NULL'], {
...
const DbColumn = Type.Object(
    {
        ...
        generated: Type.Optional(AnyOf(['PERSISTENT', 'STORED', 'VIRTUAL'])),
        genExpr: OptionalString,
        fracDigits: Type.Optional(
            Type.Number({
                minimum: 0,
                maximum: 6,
        ...
        zerofill: Type.Optional(
            Type.Union([Type.Number(), Type.Boolean()], { title: 'Zero-fill' }),
        ),
