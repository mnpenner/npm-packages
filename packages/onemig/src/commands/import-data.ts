...
export const importDataCmd = app
    ...
    .run(async ({ flags }) => {
        ...
        console.log(
            `Importing file ${Chalk.yellow(flags.filename)} into table ${Chalk.magenta(table)}`,
        )
        ...
        const colMap = Object.fromEntries(
            def.columns.map(({ name, ...columnDef }) => [name, columnDef]),
        )
        ...
        for await (const row of csvParser) {
            if (!columns) {
                columns = Object.keys(row).filter(
                    (column) => colMap[column] && !colMap[column].generated,
                )
                if (!columns.length) {
            ...
            if (batch.length >= flags.batchSize) {
                await conn.exec(
                    sql`insert into ${sql.id(table)} (${sql.columns(columns)}) values ${sql.values(batch)}`,
                )
        ...
        console.log(
            `Imported ${Chalk.cyan(rowCount)} rows from ${Chalk.yellow(flags.filename)} into ${Chalk.magenta(table)} in ${Chalk.green(elapsed)}ms`,
        )
    })
