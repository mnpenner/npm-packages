    ...
    .run(async ({ args }) => {
        ...
        for (const user of schema) {
            for (const host of toArray(user.host ?? user.hosts)) {
                ...
                const dbPrivs =
                    user.databasePrivileges ??
                    user.databasePrivs ??
                    user.dbPrivs ??
                    user.dbPrivileges
                if (dbPrivs) {
                    for (const [dbName, privileges] of Object.entries(dbPrivs)) {
                        const [databasePrivs, dbGrant] = cleanPrivileges(
                            privileges as string | string[],
                        )
                        lines.push(
                            `GRANT ${makeGrant(databasePrivs)} ON ${escapeIdStrict(dbName)}.* TO ${escapeValue(user.name)}@${escapeValue(host)}${dbGrant ? ' WITH GRANT OPTION' : ''};`,
                        )
                    }
                }
                const tblPrivs =
                    user.tablePrivileges ?? user.tblPrivs ?? user.tablePrivs ?? user.tblPrivileges
                if (tblPrivs) {
                    for (const db of Object.keys(tblPrivs)) {
                        for (const tbl of Object.keys(tblPrivs[db])) {
                            ...
                            lines.push(
                                `GRANT ${makeGrant(tablePrivs)} ON ${escapeIdStrict(db)}.${escapeIdStrict(tbl)} TO ${escapeValue(user.name)}@${escapeValue(host)}${tableGrant ? ' WITH GRANT OPTION' : ''};`,
                            )
                        }
                    }
                }
...
): [privileges: string[] | string | null, grantOption: boolean] {
    if (!privileges?.length) {
        return [null, false]
    }
