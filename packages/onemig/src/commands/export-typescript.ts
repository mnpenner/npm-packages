...
import type { DbColumn } from '../dbtypes'
import { type DbTable } from '../dbtypes'
...
const dbTsTypeMap: Record<string, Resolvable<string, [DbColumn]>> = {
    ...
    set: (col) =>
        col.values ? `Array<${col.values.map((v) => json5.stringify(v)).join('|')}>` : 'unknown',
    tinyint: 'number',
    smallint: 'number',
    mediumint: 'number',
    int: 'number',
    ...
    .run(async ({ args, flags }) => {
        ...
        for (const def of tables) {
            ...
            for (const col of def.columns) {
                ...
                if (col.comment) {
                    lines.push(
                        `  /** ${col.comment.replace(/^(UNUSED|DEPRECATED)\b/i, '@deprecated')} */`,
                    )
                }
        ...
        if (flags.mysql3 && flags.tablemap && flags.colmaps) {
            ...
            for (const def of tables) {
                ...
                lines.push(
                    `  query${tableName}<C extends keyof ${interfaceName}>(columns: C[]|null, postfix: SqlFrag|((t:SqlFrag)=>SqlFrag)) {`,
                )
                lines.push("    const t=sql.id('t')")
                lines.push(
                    `    let q=sql\`select \${sql.columns(columns??${json5.stringify(columnNames)})} from ${dumbScape(def.name)} as \${t}\``,
                )
                lines.push(
                    "    if(postfix) q=sql`\${q} \${typeof postfix === 'function' ? postfix(t) : postfix}`",
                )
                lines.push(`    return this.db.query<Pick<${interfaceName}, C>>(q)`)
                lines.push('  }')
                const pk = def.indexes.find(
                    (idx) => idx.type === 'PRIMARY' && idx.columns.length === 1,
                )
                if (pk) {
                    ...
                    lines.push(
                        `  get${name}<C extends keyof ${interfaceName}>(${varName}: ${interfaceName}[${json5.stringify(pkCol)}], columns?: C[]) {`,
                    )
                    lines.push(
                        `    return this.db.row<${interfaceName}>(sql\`select \${sql.columns(columns??${json5.stringify(columnNames)})} from ${dumbScape(def.name)} where ${dumbScape(pkCol)}=\${${varName}}\`)`,
                    )
                    lines.push('  }')
                }
            }
