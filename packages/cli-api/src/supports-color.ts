...
function hasFlag(
    flag: string,
    argv: readonly string[] = (globalThis as { Deno?: { args: string[] } }).Deno
        ? (globalThis as { Deno?: { args: string[] } }).Deno!.args
        : process.argv,
): boolean {
...
function envForceColor(): ColorSupportLevel | undefined {
    if ('FORCE_COLOR' in env) {
        ...
        return forceColor.length === 0
            ? 1
            : (Math.min(Number.parseInt(forceColor, 10), 3) as ColorSupportLevel)
    }
}
...
function supportsColorInternal(
    haveStream: boolean,
    { streamIsTTY, sniffFlags = true }: SupportsColorInternalOptions = {},
): ColorSupportLevel {
