...
function getOptionNoPrefixValue(opt: Option): any {
    ...
    throw new Error(
        `\`${getOptName(opt)}\` option must define valueIfNoPrefix when noPrefix is enabled`,
    )
}
...
function getTrailingFixedRequiredCount(
...
function ensureFiniteNumber(
    ...
    if (Number.isNaN(numericValue) || !Number.isFinite(numericValue)) {
        throw new Error(
            `Invalid value "${value}"${describeItem(itemName)} (expected ${describeOptionType(type)})`,
        )
    }
    return numericValue
}
...
function validateArgumentDefinitions(cmd: ParseableCommand): void {
    ...
    for (let i = 0; i < arguments_.length; ++i) {
        ...
        if (typeof arg.required === 'number') {
            assertValidCount(`"${arg.name}" argument required count`, arg.required, {
                allowZero: true,
            })
            if (!repeatable) {
                throw new Error(
                    `"${arg.name}" argument cannot use a numeric required count unless it is repeatable`,
                )
            }
        }
        ...
        if (repeatable && i < arguments_.length - 1) {
            if (getTrailingFixedRequiredCount(arguments_, i + 1) === undefined) {
                throw new Error(
                    'Repeatable arguments can only be followed by required arguments with fixed counts',
                )
            }
        }
        if (maxRepeatCount !== undefined && minRequired > maxRepeatCount) {
            throw new Error(
                `"${arg.name}" argument requires at least ${minRequired} values but allows at most ${maxRepeatCount}`,
            )
        }
...
function pushRepeatableValue(
    ...
    if (maxCount !== undefined && target.length >= maxCount) {
        throw new Error(
            `"${itemName}" ${kind} allows at most ${maxCount} value${maxCount === 1 ? '' : 's'}`,
        )
    }
    target.push(value)
}
...
export function formatOption(opt: Option, chalk: ChalkInstance): [string, string] {
    ...
    let flags = [
        ...aliases.map((a) => chalk.green(a.length === 1 ? `-${a}` : `--${a}`)),
        chalk.green(`--${opt.name}`),
    ].join(', ')
...
export function getValuePlaceholder(opt: Option): string {
    ...
    if (enumValues !== undefined) {
        ...
        opt.type === OptType.INPUT_DIRECTORY ||
        opt.type === OptType.OUTPUT_DIRECTORY ||
        opt.type === OptType.EMPTY_DIRECTORY
    ) {
        return 'DIR'
    } else {
...
export function validateCommandConfig(cmd: ParseableCommand): void {
    ...
    for (const opt of getOptions(cmd)) {
        ...
        if (opt.noPrefix && !isValueNotRequired(opt)) {
            throw new Error(
                `\`${getOptName(opt)}\` option cannot enable noPrefix unless valueNotRequired is enabled`,
            )
        }
        if (opt.type === OptType.ENUM && !getEnumValues(opt)?.length) {
            throw new Error(
                `\`${getOptName(opt)}\` option must define enumValues when using OptType.ENUM`,
            )
        }
...
function assignArguments(
    ...
    for (let i = 0; i < arguments_.length; ++i) {
        ...
        if (isRepeatable(def.repeatable)) {
            const trailingFixedRequiredCount =
                i < arguments_.length - 1
            ...
            if (maxRepeatCount !== undefined && valueCount > maxRepeatCount) {
                throw new Error(
                    `"${def.name}" argument allows at most ${maxRepeatCount} value${maxRepeatCount === 1 ? '' : 's'}`,
                )
            }
            ...
            for (let j = 0; j < valueCount; ++j) {
                ...
                if (def.type != null) {
                    value = coerceType(
                        ...
                        def.type,
                        chalk,
                        formatItem('argument', def.name, chalk),
                        getEnumValues(def),
                    )
                }
...
export function parseArgs(
    ...
    for (let i = 0; i < argv.length; ++i) {
        ...
        if (parseFlags && token.length >= 2 && token.startsWith('-')) {
            ...
            if (token.startsWith('--')) {
                ...
                if (!match)
                    throw new UnknownOptionError(`--${name}`, formatToken(`--${name}`, chalk))
                const { opt, negated } = match
                if (negated && inlineValue !== undefined) {
                    throw new Error(
                        `Option ${formatToken(`--no-${opt.name}`, chalk)} does not take a value`,
                    )
                }
                ...
                if (negated) {
                    ...
                    if (isValueNotRequired(opt)) {
                        ...
                        throw new Error(
                            `Missing required value for option ${formatToken(token, chalk)}`,
                        )
                    }
                }
                if (opt.type != null)
                ...
                if (isRepeatable(opt.repeatable)) {
                    pushRepeatableValue(
                        ...
                        value,
                        opt.name,
                        getMaxRepeatCount(opt.repeatable),
                        'option',
                    )
                } else opts[k] = value
            } else {
                ...
                if (hasInlineAssignment) {
                    ...
                    if (unknownOption !== undefined) {
                        throw new UnknownOptionError(
                            `-${unknownOption}`,
                            formatToken(`-${unknownOption}`, chalk),
                        )
                    }
                }
                let j = 0
                while (j < cluster.length) {
                    ...
                    else if (remainder.length && hasInlineAssignment)
                        throw new Error(
                            `Missing required value for option ${formatToken(`-${ch}`, chalk)}`,
                        )
                    else if (inlineValue !== undefined && remainder.length === 0)
                        value = inlineValue
    ...
    if (arguments_?.length) {
        for (let i = 0; i < arguments_.length; ++i) {
            ...
            const k = a.propName ?? a.name
            if (
                !isRepeatable(a.repeatable) &&
                minRequired > 0 &&
                opts[k] === undefined &&
                !skipRequiredArguments
            ) {
                throw new Error(`${formatToken(a.name, chalk)} argument is required`)
            }
            if (
                isRepeatable(a.repeatable) &&
                ((opts[k] as any[] | undefined)?.length ?? 0) < minRequired &&
                !skipRequiredArguments
            ) {
                throw new Error(
                    `\`${a.name}\` argument requires at least ${minRequired} value${minRequired === 1 ? '' : 's'}`,
                )
            }
...
function coerceType(
    ...
    const normalizedEnumValue = () => {
        ...
        if (allowedValues !== undefined && !allowedValues.includes(normalized)) {
            throw new Error(
                `Invalid value "${value}"${describeItem(itemName)} (expected one of: ${allowedValues.join(', ')})`,
            )
        }
        return normalized
    }
