...
function normalizeAppAsLeafCommand(app: AnyApp): AnyLeafCommand {
    ...
    return {
        ...
        ...(getCommandArguments(app) !== undefined
            ? { arguments: [...getCommandArguments(app)!] }
            : {}),
        execute: handler,
    }
}
...
function normalizeLeafCommand(cmd: AnyLeafCommand): AnyLeafCommand {
    ...
    return {
        ...
        ...(getCommandArguments(cmd) !== undefined
            ? { arguments: [...getCommandArguments(cmd)!] }
            : {}),
        execute: handler,
    }
}
...
function getRequestedColorMode(app: AnyApp, argv: readonly string[]): ColorMode {
    ...
    const longNames = new Set<string>([
        colorOption.name,
        ...(Array.isArray(colorOption.alias)
            ? colorOption.alias
            : colorOption.alias !== undefined
              ? [colorOption.alias]
              : []),
    ])
    ...
    for (const token of argv) {
        ...
        if (token.startsWith('-') && !token.startsWith('--')) {
            ...
            const cluster = clusterText.includes('=')
                ? clusterText.slice(0, clusterText.indexOf('='))
                : clusterText
...
function parseGlobalOptions(
    app: AnyApp,
    argv: string[],
): { opts?: Record<string, any>; result?: ExecutionResult } {
...
function getOptionTokenConsumption(
    ...
    const matchLong = (name: string) =>
        options.find(
            (opt) =>
                opt.name === name ||
                (opt.noPrefix && `no-${opt.name}` === name) ||
                (opt.alias !== undefined &&
                    (Array.isArray(opt.alias) ? opt.alias.includes(name) : opt.alias === name)),
        )
...
function unknownCommandResult(
    ...
    const error = createError(
        `${getProcName(app)}: unknown command ${formatToken(commandName, chalk)}`,
        ErrorCategory.InvalidArg,
    )
    return {
        code: getErrorExitCode(error),
...
async function executeAppDetailed(
    app: AnyApp,
    ...
    if (hasSubCommands(resolved.command)) {
        ...
        if (
            !resolved.remainingArgv.length ||
            (branchOpts.help && firstNonGlobalToken === undefined)
        ) {
            printCommandHelp(context, resolved.command, resolved.path)
            return { result: { code: 0 }, context }
        }
        return {
            result: unknownTokenResult(
                app,
                firstNonGlobalToken ?? resolved.remainingArgv[0],
                context.chalk,
            ),
            context,
        }
 ...
 */
export async function executeAppResult(
    app: AnyApp,
    argv: string[] = process.argv.slice(2),
