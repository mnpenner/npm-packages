...
test(flatMap.name, () => {
    ...
    expect(flatMap([1, 2], (x) => x * 2)).toEqual([2, 4])
})

test(reduceArray.name, () => {
    expect(reduceArray([0, 1, 2, 3], (acc, val) => acc + val)).toEqual(6)
    expect(reduceArray([0, 1, 2, 3], (acc, val) => acc + val, 0)).toEqual(6)
})
...
test(groupBy.name, () => {
    ...
    expect(groupBy(people, (p) => p.species)).toEqual({
        Human: [
            {
                name: 'Luke Skywalker',
                species: 'Human',
    ...
    expect(groupBy(new Set([6.1, 4.2, 6.3]), Math.floor)).toEqual({ '4': [4.2], '6': [6.1, 6.3] })
})
