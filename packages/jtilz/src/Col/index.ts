 ...
 */
export function filterMap<TVal, TRet>(
    dict: IDictionary<TVal>,
    callback: (v: TVal, k: string, d: IDictionary<TVal>) => TRet | symbol,
): IDictionary<TRet>
export function filterMap<TVal, TRet>(
    iter: Iterable<TVal>,
    callback: (v: TVal, k: number, i: Iterable<TVal>) => TRet | symbol,
): TRet[]
 ...
 */
export function mapArray<TVal, TRet>(
    iterable: Iterable<TVal>,
    callback: MapCallback<TVal, TRet>,
): TRet[] {
    const out = []
    let i = 0
 ...
 */
export function filterArray<TVal>(
    iterable: Iterable<TVal>,
    callback: (value: TVal, index: number, iterable: Iterable<TVal>) => boolean,
): TVal[] {
    const out = []
    let i = 0
    for (const x of iterable) {
 ...
 */
export function filterAsync<TInput, TResult>(
    iterable: Iterable<TInput>,
    mapCb: MapCallback<TInput, Promise<TResult>>,
    filterCb: (el: TResult) => boolean = (el: any) => !!el,
): Promise<TInput[]> {
    return Promise.all(mapArray(iterable, mapCb)).then((results) =>
        filterArray(iterable, (_, i) => filterCb(results[i])),
    )
}
...
export function filterMapAsync<TVal, TRet>(
    ...
    return allSettled(mapArray(iterable, callback)).then((array) =>
        filterMap(array, (r) => (r.state === FULFILLED ? r.value : SKIP)),
    )
}
...
export function groupBy<T>(
    ...
    return reduceArray<T, IDictionary<T[]>>(
        iterable,
        (acc, val, idx) => {
