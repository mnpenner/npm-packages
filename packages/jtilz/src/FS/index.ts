 ...
 */
export const readDir = (path: string) => readdirAsync(path).then(entries => entries.map(e => Path.join(path, e)));
 ...
 */
export const fileAccess = (path: string|Buffer|URL, mode: number) => accessAsync(path, mode).then(() => true, err => {
