  ...
  return obj
    .replaceAll(
      ...
      (m) =>
        '"' +
        Array.from(m)
          .map((c) => DASH_MAP.get(c) ?? '\\0' + c.codePointAt(0)!.toString(8))
          .join('') +
        '"',
    )
    .replaceAll(' ', '\\ ')
...
function escapeTemplate(
  [cmdStart, ...cmdRest]: TemplateStringsArray,
  args: CommandArg[],
  escapeStr: StringEscapeFn,
) {
...
function execParse<T>(
  cmdParts: TemplateStringsArray,
  args: CommandArg[],
  ...
  return new ErrPromise<T, ExecError>((resolve, reject) => {
    ...
    const proc = exec(cmd, { shell }, (error, stdout, stderr) => {
      ...
      if (error !== null) {
        ...
        fixed.message = (
          error.message.endsWith(stderr) ? error.message.slice(0, -stderr.length) : error.message
        ).trimEnd()
        fixed.stdout = stdout
        fixed.stderr = stderr
...
export function awk(cmd: TemplateStringsArray, ...args: CommandArg[]) {
  return execParse(cmd, args, (stdout) =>
    stdout
      .replace(/\n$/, '')
      .split('\n')
