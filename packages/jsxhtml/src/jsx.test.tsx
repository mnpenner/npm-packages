...
it('supports custom types', () => {
    expect(String(<C>some comment</C>)).toEqual(
        process.env.NODE_ENV === 'production' ? '' : '<!-- some comment -->',
    )
    expect(String(<DocType html />)).toEqual(
...
it('builds style strings', () => {
    ...
    ).toEqual('<span style=border-right:2px;border-left:0;border-bottom-color:red;>x</span>')
})
...
it('renders exotic attributes', () => {
    expect(String(<input type="color" colorspace="display-p3" />)).toEqual(
        '<input type=color colorspace=display-p3>',
    )
    expect(String(<input type="number" step={1n} min={0n} max={42n} />)).toEqual(
        '<input type=number step=1 min=0 max=42>',
    )
    ...
    expect(String(<img src="/hero.jpg" width={640} height={'360'} loading="lazy" />)).toEqual(
        '<img src=/hero.jpg width=640 height=360 loading=lazy>',
    )
})
