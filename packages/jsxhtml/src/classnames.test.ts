...
test('supports objects', () => {
    ...
    expect(classCat({ foo: true }, { bar: false }, null, { '--foobar': 'hello' })).toBe(
        'foo --foobar',
    )
})
...
test('supports arrays', () => {
    ...
    expect(classCat(['foo'], ['', 0, false, 'bar'], [['baz', [['hello'], 'there']]])).toBe(
        'foo 0 bar baz hello there',
    )
})
...
test('matches classnames-style usage', () => {
    ...
    expect(classCat({ foo: true, bar: true })).toBe('foo bar')
    expect(classCat('foo', { bar: true, duck: false }, 'baz', { quux: true })).toBe(
        'foo bar baz quux',
    )
