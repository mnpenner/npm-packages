...
async function crossSpawn(
    cmd: string,
    args: string[],
    opts: Omit<childProc.SpawnOptions, 'shell' | 'windowsVerbatimArguments'>,
) {
    if (isWindows) {
...
async function main(mainArgs: string[]): Promise<number | void> {
    ...
    async function resolveRule(ruleName: string, depth: number = 0): Promise<number> {
        ...
        const odeps = toStringArray(
            rule.orderedDependencies ?? rule.odeps ?? rule.prerequisites ?? rule.prereqs,
        )
        ...
        const makeDir = toStringArray(
            rule.mkdir ?? rule.mkdirp ?? rule.makeDir ?? rule.makeDirectory,
        ) // TODO: allow `true` to create any directories for output files
        ...
        if (deps.length) {
            ...
            if (failedDeps.length) {
                info(
                    `${failedDeps.length} dependencies failed; aborting`,
                    Object.fromEntries(failedDeps),
                )
        ...
        const lastSuccessfulBuildNanos: bigint =
            lastSuccessfulBuildMs != null
                ? msToNs(lastSuccessfulBuildMs)
        ...
        if (doesNotContainNull(outputTimes)) {
            ...
            if (inputLastMod) {
                ...
                const oldestOutputModTime = allOutputTimes.length
                    ? min(allOutputTimes)!
                    : lastSuccessfulBuildNanos
            ...
            if (Array.isArray(cmd)) {
                ...
                if (shell) {
                    ...
                    if (output.length) {
                        cmd = cmd.replace(
                            /(?<!\\)\$@(?![$@%<>?^+|*&!])/g,
                            output.map((o) => escapeShellArg(o)).join(' '),
                        )
                    }
                }
            }
        ...
        if (shell) {
            ...
            info(
                Chalk.cyanBright.bold('$ ') +
                    Chalk.green.underline(cmd) +
                    cmdArgs.map((x) => ' ' + Chalk.underline(x)).join(''),
            )
        }
        ...
        const code = await spawn(ruleName, cmd, cmdArgs, {
            suppressOutput,
            interactive: Boolean(rule.interactive != null ? rule.interactive : interactive),
            cwd,
            shell,
        })
        const elapsed = nsToMs(hrtime.bigint() - start)
        info(
            `Exited with code ${Chalk[code === 0 ? 'green' : 'red'](code)} in ${Chalk.blue(elapsed)}ms`,
        )
        if (code === 0) {
            cache.rules ??= {}
