...
describe('example3 router', () => {
    it('serializes successful responses as JSON by default', async () => {
        const response = await router.fetch(
            new Request('https://example.com/widgets/7?view=summary', {
    ...
    it('serializes successful responses as YAML when requested', async () => {
        const response = await router.fetch(
            new Request('https://example.com/widgets/7?view=full', {
                method: HttpMethod.POST,
                headers: {
        ...
        expect(response.status).toBe(HttpStatus.OK)
        expect(response.headers.get('content-type')).toBe(CommonContentTypes.YAML)
        const body = await response.text()
        expect(body).toContain('summary:')
        expect(body).toContain('Widget 7: alpha (2 tag(s))')
    })

    it('serializes validation errors using the same content negotiation', async () => {
        const response = await router.fetch(
            new Request('https://example.com/widgets/not-a-number?view=summary', {
                method: HttpMethod.POST,
                headers: {
                    accept: CommonMediaTypes.YAML,
                    'content-type': CommonContentTypes.JSON,
                },
