...
import type { PartialRecord } from './util'
import type { BunUrl, Handler } from './server-api'
...
export function serve({ timeout, router, log, listen, ...options }: SimpleServeOptions) {
    const server = Bun.serve({
        ...
        async fetch(req) {
            ...
            if (log)
                console.log(
                    Chalk.grey(`>${reqId}`) +
                        ` ${Chalk.whiteBright.bold(req.method)} ${path} | ${formatSize(req.headers.get('Content-Length'))} | ${formatContentType(req.headers.get('Content-Type'))}`,
                )
                ...
                if (handler) {
                    ...
                    bunRes.bodyPromise.then(() => {
                        ...
                        if (log)
                            console.log(
                                Chalk.grey(`<${reqId}`) +
                                    ` ${formatStatus(bunRes.status)} | ${formatSize(bunRes.headers.get('content-length'))} | ${formatContentType(bunRes.headers.get('content-type'))} | ${formatDuration(headerTime)}+${formatDuration(bodyExtra)}`,
                            )
                    })
    ...
    if (listen) {
        ...
        console.log(
            `Listening on ${server.hostname}:${server.port} (http://localhost:${server.port})`,
        )
    }
}
