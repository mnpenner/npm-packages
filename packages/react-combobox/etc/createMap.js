...
async function main() {
    ...
    lineReader.on('line', (line) => {
        ...
        const weights = collElement
            .split('][')
            .map((w) => /[0-9A-F]+/.exec(w)[0])
        ...
        if (weights.length) {
            ...
            if (cp <= 0x1fff) {
                // TypeScript craps out if we make this too large.
    ...
    lineReader.on('close', () => {
        fs.writeFileSync(
            `${__dirname}/../src/charMap.ts`,
            `const $: {[ch:string]:number[]} = ${serialize(mapping)};\nexport default $;`,
            { encoding: 'utf8' },
        )
    })
}
