    ...
    describe('Custom tests', () => {
        ...
        it('matches schedule w/ extraneous vars', () => {
            const templ = new UriTemplate(
                '/schedule/{year:int:4}-{month:int:2}-{day:int:2}{?foo,q*}',
            )
            const match = templ.match('/schedule/2019-12-31?foo=bar&baz=bux')
            expect(match).toEqual({
                score: 16,
                params: { year: 2019, month: 12, day: 31, foo: 'bar', q: { baz: 'bux' } },
            })
        })
        ...
        it('explode', () => {
            ...
            expect(new UriTemplate('find{?year*}').expand(vars)).toEqual(
        ...
        describe('Expression Expansion', () => {
            ...
            it('Variable Expansion', () => {
                ...
                expect(new UriTemplate('{;count*}').expand(vars)).toEqual(
            ...
            it('Variable Expansion', () => {
                ...
                expect(new UriTemplate('{x,hello,y}').expand(vars)).toEqual(
            ...
            it('Variable Match', () => {
                expect(new UriTemplate('{list}').match('red,green,blue')?.params).toEqual({
                    list: 'red,green,blue',
                })
                expect(new UriTemplate('{list*}').match('red,green,blue')?.params).toEqual({
                    list: ['red', 'green', 'blue'],
                })
                expect(new UriTemplate('{keys}').match('semi,%3B,dot,.,comma,%2C')?.params).toEqual(
                    { keys: 'semi,;,dot,.,comma,,' },
                ) // Not so sure about this one...
                expect(
                    new UriTemplate('{keys*}').match('semi=%3B,dot=.,comma=%2C')?.params,
                ).toEqual({ keys: ['semi=;', 'dot=.', 'comma=,'] })
            })
            ...
            it('Reserved Expansion: {+var}', () => {
                ...
                expect(new UriTemplate('{+half}').expand(vars)).toEqual('50%25')
                expect(new UriTemplate('{base}index').expand(vars)).toEqual(
                ...
                expect(new UriTemplate('here?ref={+path}').expand(vars)).toEqual(
                    'here?ref=/foo/bar',
                )
                expect(new UriTemplate('up{+path}{var}/here').expand(vars)).toEqual(
