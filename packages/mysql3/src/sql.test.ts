...
describe('sql', () => {
    it('autoescapes values', () => {
        ...
        expectSql(
            sql`select * from t where x in (${[1, 2, 'x']})`,
            "select * from t where x in (1,2,'x')",
        )
        expectSql(
            sql`select ${new Date('2021-01-31T07:10:01.302Z')}`,
            `select TIMESTAMP'2021-01-31 07:10:01.302'`,
        )
        expectSql(
            sql`select ${new Date('2021-01-31T07:10:01.310Z')}`,
            `select TIMESTAMP'2021-01-31 07:10:01.310'`,
        )
        expectSql(
            sql`select ${new Date('2021-01-31T07:10:01.000Z')}`,
            `select TIMESTAMP'2021-01-31 07:10:01'`,
        )
    })
    ...
    it('alias', () => {
        expectSql(
            sql`select ${sql.alias({ a: 'b', 'c.d': '4', e: sql`count(*)` })}`,
            'select `b` AS `a`, `4` AS `c.d`, count(*) AS `e`',
        )
        expectSql(
            sql`select ${sql.alias([
                ['a', 'b'],
                [['c', 'd'], 'e'],
                ['f.g', 'h'],
            ])}`,
            'select `a` AS `b`, `c`.`d` AS `e`, `f.g` AS `h`',
        )
    })
    ...
    it('prevents Shift-JIS attack', () => {
        ...
        expectSql(
            sql`SELECT *
                      ...
                      WHERE bar = ${input}
                      LIMIT 1`,
            "SELECT * FROM foo WHERE bar = '\x81'' OR 1=1 #' LIMIT 1",
        )
    })
    ...
    it('ids', () => {
        ...
        expectSql(
            sql`select ${sql.col(['db', 'tbl', 'col'])}
                      from dual`,
            'select `db`.`tbl`.`col` from dual',
        )
    })
    ...
    it('values', () => {
        expectSql(
            sql`insert into t (foo, bar)
                      values ${sql.values([
                          [1, '2'],
                          [3, '4'],
                      ])}`,
            "insert into t (foo,bar) values (1,'2'),(3,'4')",
        )
    })
    ...
    it('insert', () => {
        ...
        expectSql(
            sql.insert('t', { a: 1, 'b.c': 'x' }, { ignore: true }),
            "INSERT IGNORE INTO `t` SET `a`=1, `b`.`c`='x'",
        )
        expectSql(
            sql.insert('t', { a: 1, 'b.c': 'x' }, { onDuplicateKey: DuplicateKey.IGNORE }),
            "INSERT INTO `t` SET `a`=1, `b`.`c`='x' ON DUPLICATE KEY UPDATE `a`=`a`",
        )
        expectSql(
            sql.insert('t', { a: 1, 'b.c': 'x' }, { onDuplicateKey: DuplicateKey.UPDATE }),
            "INSERT INTO `t` SET `a`=1, `b`.`c`='x' ON DUPLICATE KEY UPDATE `a`=VALUES(`a`), `b`.`c`=VALUES(`b`.`c`)",
        )
        ...
        expectSql(
            ...
            "INSERT INTO `t` SET `a`=1, `b.c`='x', `e`.`f`='y'",
        )

        expectSql(
            sql.insert<FakeTable>('users', {
                id: 1,
                name: 'mpen',
