...
export class ConnectionPool<TDefaultValue = DefaultValueType> {
    ...
    private _fwd<K extends keyof typeof PoolConnection.prototype>(
        method: K,
    ): (typeof PoolConnection.prototype)[K] {
    ...
    async transaction<TReturn>(
        callback: (conn: PoolConnection<TDefaultValue>) => Promise<TReturn>,
    ): Promise<TReturn>
    async transaction<TUnionResults = Record<string, TDefaultValue>>(
        sqlFrags: SqlFrag[],
        ...
        if (Array.isArray(callback)) {
            return this.transaction<any>(async (conn) => {
                ...
                if (results.some((r) => r.status === 'rejected')) {
                    const errors = zip(callback, results)
                        .map((x, i) => ({
                            index: i,
                            query: x[0],
                            result: x[1],
                    ...
                    throw Error(
                        `${errors.length} quer${errors.length === 1 ? 'y' : 'ies'} failed:${errors.map((err) => `\n[${err.index}] ${err.query.toSqlString()} :: ${(err.result as any).reason}`).join('')}`,
                    )
                }
