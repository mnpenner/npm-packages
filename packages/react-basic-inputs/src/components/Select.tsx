...
export type SelectOption<T> = OverrideProps<
    'option',
    {
        value: T
        text: ReactNode
...
export type SelectProps<T extends NonNil> = OverrideProps<
    'select',
    {
...
export function Select<T extends NonNil>({
    ...
    const refreshSelectedIndex = useCallback(() => {
        ...
        if (
            ...
            fixedOptions[ref.current.selectedIndex].value != value
        ) {
    ...
    return (
        <select {...selectAttrs} onChange={handleChange} ref={setRef}>
            {fixedOptions.map((opt, idx) => {
                ...
                return (
                    <option {...optAttrs} key={fixedKey} value={fixedKey}>
                        {opt.text}
                    </option>
                )
            })}
