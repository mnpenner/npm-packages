    ...
    const parser = new htmlparser.Parser(
        {
            onopentag(tagName, attrs) {
                ...
                if (attrs.hasOwnProperty('style')) {
                    throw new Error(
                        `Sorry, "style" attributes are not presently supported (requires parsing CSS)`,
                    )
                }
                for (const attrName of Object.keys(attrs)) {
                    if (stdAttrs.hasOwnProperty(attrName) && attrName !== stdAttrs[attrName]) {
                        if (attrs.hasOwnProperty(stdAttrs[attrName])) {
                            loader.emitWarning(
                                new Error(
                                    `<${tagName}> has both "${attrName}" and "${stdAttrs[attrName]}" attributes`,
                                ),
                            )
                        } else {
                ...
                if (isSvg)
                    sb.push(
                        ',',
                        ['title', 'desc']
                            .map((a) => `${a}!=null?e(${toJs(a)},null,${a}):null`)
                            .join(','),
                    )
            },
            onclosetag(name) {
